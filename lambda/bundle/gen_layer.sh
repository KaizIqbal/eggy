...
docker run --rm -v "$PWD"/"$NAME":/opt lambci/lambda:build-python3.8 \
  bash -c "yum install -y libX11-devel libpng-devel libXcursor-devel libcurl-devel && pip install ${PACKAGES} -t /opt/python && export PYCURL_SSL_LIBRARY=openssl && pip install pycurl --no-cache-dir -t /opt/python"

docker run --rm -v "$PWD"/"$NAME":/lambda/opt lambci/yumda:2 yum install -y libX11 libpng libXcursor libcurl openssl gnutls
